...
def GetTerminationConditions():
    ...
    if len(config.objective_IDs) == 1:
        ...
        term_conditions = TerminationCollection(RobustTermination(SingleObjectiveSpaceTermination(tol=1E-6, 
                                                                                                  only_feas=True), 
                                                                                                  period=20),  # Chance in objective value termination condition
                                                ...
                                                RobustTermination(DesignSpaceTermination(tol=1E-12), 
                                                                  period=20),  # Maximum change in design vector termination condition
                                                RobustTermination(ConstraintViolationTermination(tol=1E-12, terminate_when_feasible=False), 
                                                                  period=20)  # Maximum change in constriant violation termination condition
                                                )
    else:
        ...
        term_conditions = TerminationCollection(RobustTermination(MultiObjectiveSpaceTermination(tol=1E-6, 
                                                ...
                                                RobustTermination(DesignSpaceTermination(tol=1E-12), 
                                                                  period=20),  # Maximum change in design vector termination condition
                                                RobustTermination(ConstraintViolationTermination(tol=1E-12, terminate_when_feasible=False), 
                                                                  period=20)  # Maximum change in constraint violation termination condition
                                                )
