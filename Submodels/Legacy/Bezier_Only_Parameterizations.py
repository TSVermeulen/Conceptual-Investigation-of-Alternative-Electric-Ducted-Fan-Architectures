...
class AirfoilParameterization():
    ...
    def GetReferenceParameters(self) -> None:
        ...
        self.r_LE = -1 * result.x[-1]  # LE radius multiplied by -1 to comply with sign convention of method

        # Calculate the leading edge direction gamma_LE, trailing wedge angle beta_TE, and trailing camber line angle alpha_TE
        self.leading_edge_direction = np.atan(self.camber_gradient_distribution[0])
        self.trailing_wedge_angle = np.atan(-1 * self.thickness_gradient_distribution[-1])  # Multiply gradient by -1 to comply with sign convention
        self.trailing_camberline_angle = np.atan(-1 * self.camber_gradient_distribution[-1])  # Multiply gradient by -1 to comply with sign convention
    ...
    def GetThicknessControlPoints(self,
        ...
        x_trailing_edge_thickness_coeff = np.array([self.x_t,
                                                    (7 * self.x_t + (9 * b_8 ** 2) / (2 * self.r_LE)) / 4,
                                                    3 * self.x_t + (15 * b_8 ** 2) / (4 * self.r_LE),
                                                    b_15,
                                                    1])
        ...
        y_trailing_edge_thickness_coeff = np.array([self.y_t,
                                                    self.y_t,
                                                    (self.y_t + b_8) / 2,
                                                    self.dz_TE + (1 - b_15) * np.tan(self.trailing_wedge_angle),
                                                    self.dz_TE])
    ...
    def GetCamberControlPoints(self,
        ...
        y_leading_edge_camber_coeff = np.array([0,
                                                b_0 * np.tan(self.leading_edge_direction),
                                                self.y_c,
                                                self.y_c])
        
        # Construct trailing edge x coefficients
        x_trailing_edge_camber_coeff = np.array([self.x_c,
                                                 (3 * self.x_c - self.y_c / np.tan(self.leading_edge_direction)) / 2,
                                                 (-8 * self.y_c / np.tan(self.leading_edge_direction) + 13 * self.x_c) / 6,
                                                 b_17,
                                                 1])
        ...
        y_trailing_edge_camber_coeff = np.array([self.y_c,
                                                 self.y_c,
                                                 5 * self.y_c / 6,
                                                 self.z_TE + (1 - b_17) * np.tan(self.trailing_camberline_angle),
                                                 self.z_TE])
    ...
    def FindInitialParameterization(self, 
        ...
        Returns
        -------
        b_0, b_2, b_8, b_15, b_17 : tuple[float, float, float, float, float]
            Optimized Bezier control points for the airfoil parameterization.
        """

        def Objective(x: list[float, float, float, float, float],
                      plot: bool = False
                      ) -> float:
            """
        ...
        upper_bounds = [min(self.y_t, np.sqrt(-2 * self.r_LE * self.x_t / 3)) - 1E-5, np.inf, np.inf, np.inf, np.inf]
        lower_bounds = [1E-5, -np.inf, -np.inf, -np.inf, -np.inf]
        bounds = (lower_bounds, upper_bounds)

        # Perform non-linear least-squares regression
        guess_design_vector = [0.7 * upper_bounds[0], 0.75, self.y_c, self.y_c, 0.7]  # [b_8, b_15, b_0, b_2, b_17]
        ...
        # Generate
        if plot:
            CheckOptimizedResult(b_0, b_2, b_8, b_15, b_17)

        return b_0, b_2, b_8, b_15, b_17


if __name__ == "__main__":
    call_class = AirfoilParameterization()

    b_0, b_2, b_8, b_15, b_17 = call_class.FindInitialParameterization('whitcomb.dat',
                                                                       plot=True)
